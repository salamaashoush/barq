...
import {
  boundary as ssrBoundary,
  html as ssrHtml,
  renderPage,
  renderToStream,
} from "@barqjs/server";
const HY = 4;
const ssrLoading = (s: never, p: { fallback?: unknown; children: unknown }) =>
  ssrBoundary(s, null, null, "loading", p.fallback as never, p.children as never, HY);
const ssrErrored = (s: never, p: { fallback: unknown; children: unknown }) =>
  ssrBoundary(s, null, null, "error", p.fallback as never, p.children as never, HY);
