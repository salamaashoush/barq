      ...
      const children =
        typeof props.children === "function"
          ? props.children.length > 0
            ? (props.children as (item: NonNullable<T>) => Child)(value as NonNullable<T>)
            : (props.children as () => Child)()
          : props.children;
      const nodes = childToNodes(children as Child);
...
function reconcileNodes(
  ...
  for (let i = 0; i < newLen; i++) {
    sources[i] = oldKeyIndex.get(newKeys[i]) ?? -1;
  }
...
export function Index<T, U extends JSXElement>(props: {
  ...
  effect(() => {
    ...
    if (currentLength === 0 && props.fallback != null) {
      clearRange(startMarker, endMarker);
    }
