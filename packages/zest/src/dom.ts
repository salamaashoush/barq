...
function applyProp(element: Element, key: string, value: unknown, isSvg: boolean): void {
  ...
  if (typeof value === "function") {
    effect(() => {
      applyResolvedProp(element, key, (value as () => unknown)(), isSvg);
    });
    return;
  }
...
function applyResolvedProp(element: Element, key: string, value: unknown, isSvg: boolean): void {
  ...
  if (key === "style") {
    if (typeof value === "object" && value !== null) {
      applyStyles(element as HTMLElement, value as Record<string, unknown>);
  ...
  // Dangerous innerHTML
  if (key === "dangerouslySetInnerHTML" && typeof value === "object" && value !== null) {
    (element as HTMLElement).innerHTML = (value as { __html: string }).__html ?? "";
    return;
  }
...
function applyClass(element: Element, value: unknown): void {
  ...
  if (typeof value === "object") {
    let className = "";
    for (const k in value as Record<string, boolean>) {
      if ((value as Record<string, boolean>)[k]) {
...
function setProp(element: Element, key: string, value: unknown, isSvg: boolean): void {
  ...
  if (isSvg) {
    element.setAttribute(propKey, String(value));
    return;
  }

  // DOM properties (value, checked, etc.) set directly
  if (propKey in DOM_PROPS) {
    (element as unknown as Record<string, unknown>)[propKey] = value;
    return;
  }
...
function applyStyles(element: HTMLElement, styles: Record<string, unknown>): void {
  ...
  for (const prop in styles) {
    ...
    if (typeof value === "function") {
      ...
      effect(() => {
        setStylePropDirect(style, cssProperty, prop, (value as () => unknown)());
      });
    } else {
