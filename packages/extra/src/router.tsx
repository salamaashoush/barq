 ...
 */
export type NavigationGuard = (ctx: NavigationGuardContext) => boolean | string | Promise<boolean | string>
...
function cleanLoaderCache(state: RouterState): void {
  ...
  if (state.loaderCache.size > maxSize) {
    const entries = Array.from(state.loaderCache.entries()).sort((a, b) => a[1].timestamp - b[1].timestamp)
...
async function performCoreNavigation(state: RouterState, to: string, options: NavigateOptions = {}): Promise<void> {
  ...
  debug('Navigation match:', match ? { route: match.route.path, parents: match.parents.map((p) => p.path) } : null)
...
function setupBrowserListeners(state: RouterState): () => void {
  ...
  const handlePopstate = () => {
    ...
    performCoreNavigation(state, loc.pathname + loc.search + loc.hash, { noScroll: false })
  }
...
export function useSearchParams(): [
  ...
  const setSearchParams = (params: Record<string, string> | ((prev: SearchParams) => Record<string, string>)) => {
    const current = location().searchParams
    const newParams = typeof params === 'function' ? params(current) : params

    // Filter out empty values
    const filteredParams = Object.fromEntries(Object.entries(newParams).filter(([_, v]) => v !== ''))
...
export async function prefetch(path: string): Promise<void> {
  ...
  await Promise.all(
    allRoutes.map((route) => executeLoader(state, route, match.params, searchParams, signal).catch(() => {})),
  )
}
...
function setupPrefetch(
  ...
  if (strategy === 'hover') {
    ...
    element.addEventListener(
      'mouseenter',
      () => {
        timeout = setTimeout(() => prefetch(href), delay)
      },
      { signal },
    )
    element.addEventListener(
...
export function Link(props: LinkProps): JSXElement {
  ...
  const handleClick = (e: MouseEvent) => {
    ...
    e.preventDefault()
    e.stopPropagation()
    state.navigate(resolvedHref(), { replace: props.replace })
  }
...
export function Outlet(): JSXElement {
  ...
  const renderRoute = () => {
    ...
    if (route === prevRoute && data === prevData && error === prevError && currentNodes.length > 0) {
    ...
    if (error && route.errorElement) {
      createScope((dispose) => {
        ...
        const resetFn = () => {
          ...
          state.navigate(state.location().pathname, { replace: true })
        }
...
function useRouteMatching(state: RouterState): void {
  ...
  performCoreNavigation(state, loc.pathname + loc.search + loc.hash, { noScroll: true })
...
export function Router(props: RouterProps): JSXElement {
  ...
  if (initialMatch) {
    ...
    const hasLoaders = allRoutes.some((r) => r.loader)
    if (hasLoaders) {
      performCoreNavigation(state, initialLoc.pathname + initialLoc.search + initialLoc.hash, {
        noScroll: true,
      })
    }
  }
...
export function MemoryRouter(props: MemoryRouterProps): JSXElement {
  ...
  if (initialMatch) {
    ...
    const hasLoaders = allRoutes.some((r) => r.loader)
    if (hasLoaders) {
      performCoreNavigation(state, initialPath, { noScroll: true })
    }
  }
...
export function defineRoute<T, P extends Params = Params>(definition: RouteDefinition<T, P>): RouteDefinition<T, P> {
  return definition
}
...
export function Redirect(props: RedirectProps): null {
  ...
  useEffect(() => {
    state.navigate(props.to, { replace: props.replace ?? true })
  })
