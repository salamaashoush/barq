...
async function performNavigationWithTransition(
  ...
  try {
    ...
    const transition = (document as { startViewTransition: (cb: () => Promise<void>) => { finished: Promise<void> } }).startViewTransition(() => performNavigation());
...
async function performCoreNavigation(
  ...
  debug("Navigation hasLoaders:", hasLoaders, "routes:", allRoutes.map(r => ({ path: r.path, hasLoader: !!r.loader })));
  if (hasLoaders) {
