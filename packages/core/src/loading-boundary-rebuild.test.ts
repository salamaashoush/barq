...
describe("M7's parked-boundary bug", () => {
  test("a nested region that swaps while the body is stale reaches the document once", async () => {
    ...
    const arms = [
      block((_s: Scope | null) => { const n = document.createElement("b"); n.textContent = "ARM0"; return n }),
      block((_s: Scope | null) => { const n = document.createElement("i"); n.textContent = "ARM1"; return n }),
    ]
    const body = block((s: Scope | null) => {
      try { data() } catch (err) { if (err instanceof NotReadyError) throw err; throw err }
      return branch(s, null, null, () => which(), arms)
    })
    scope(() => {
      const el = boundary(null, null, null, "loading",
        block(() => document.createTextNode("WAIT")), body, 0, () => source())
      render(el, container)
    })
    await tick()
    first.resolve("one")
