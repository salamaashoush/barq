...
describe("deep", () => {
  test("returns a plain deep copy, not proxies", () => {
    const [state] = store({ user: { name: "Ada", tags: ["x"] } });
  ...
  test("leaves non-wrappable values alone", () => {
    ...
    const [state] = store({ when: date, n: 1 });
    const snap = deep(state);
...
describe("storePath", () => {
  ...
  test("DELETE removes the property", () => {
    const [state, setState] = store<{ user: { name: string; nick?: string } }>({
  ...
  test("an array of keys writes several siblings", () => {
    const [state, setState] = store({ a: 0, b: 0, c: 0 });
  ...
  test("a range targets a slice", () => {
    const [state, setState] = store({ nums: [0, 1, 2, 3, 4] });
  ...
  test("missing intermediate paths are a no-op, not a crash", () => {
    const [state, setState] = store<{ a?: { b?: { c?: number } } }>({});
