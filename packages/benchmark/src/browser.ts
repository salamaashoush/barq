...
const barqWinsEl = document.getElementById("barq-wins") as HTMLDivElement;
...
function renderResults(
  results: Map<string, { barq?: BenchmarkResult; solid?: BenchmarkResult }[]>,
) {
  let html = "";
  ...
  for (const [suiteName, benchmarks] of results) {
    ...
    for (const { barq, solid } of benchmarks) {
      const operation = barq?.operation || solid?.operation || "";
      const barqMs = barq ? (barq.avgMs < 0 ? "ERROR" : `${barq.avgMs.toFixed(3)} ms`) : "-";
      ...
      html += `<div class="benchmark-row">
        ...
        <div class="metric barq">${barqMs}</div>
...
function updateSummary(
  ...
  barqWinsEl.textContent = String(barqWins);
...
async function runBenchmarks(suitesToRun: BenchmarkSuite[]) {
  ...
  const results = new Map<string, { barq?: BenchmarkResult; solid?: BenchmarkResult }[]>();

  let totalBenchmarks = 0;
  for (const suite of suitesToRun) {
    totalBenchmarks += suite.benchmarks.length * 2; // barq + solid
  }

  let completed = 0;

  for (const suite of suitesToRun) {
    const suiteResults: { barq?: BenchmarkResult; solid?: BenchmarkResult }[] = [];

    for (const bench of suite.benchmarks) {
      const pair: { barq?: BenchmarkResult; solid?: BenchmarkResult } = {};

      // Run barq
      updateProgress(completed, totalBenchmarks, `${suite.name}: ${bench.operation} (barq)`);
      await new Promise((r) => setTimeout(r, 10)); // Let UI update

      try {
        pair.barq = await benchmark(
        ...
        console.error(`Barq ${bench.operation} failed:`, e);
