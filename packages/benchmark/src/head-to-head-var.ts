 ...
 * Byte-identical to head-to-head.ts except that the barq build is chosen by
 * BARQ_A, so the same instrument can be pointed at a second signals.ts.
 * Run: BARQ_A=<path> bun run src/head-to-head-var.ts
 */
const {
  computed: bComputed,
  createScope: bScope,
  ...
  flush: bFlush,
  signal: bSignal,
} = (await import(process.env.BARQ_A ?? "@barqjs/core")) as typeof import("@barqjs/core");
import {
...
console.log(`# ${process.env.BARQ_A ?? "@barqjs/core"}`);
console.log(
