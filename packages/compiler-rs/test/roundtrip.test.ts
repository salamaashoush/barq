...
describe("emitted code snapshots", () => {
  ...
  /**
   * C2's third evidence: a component this module names only as an OBJECT
   * PROPERTY's value.
   *
   * TanStack's authoring surface never writes `<Layout />` and never exports
   * it — `createRootRoute({ component: Layout })` is the whole of the evidence.
   * Without this the module has no component at all, compiles against a null
   * scope, and every context read in it silently answers the default: measured
   * on the reference application as a correct `<head>` above an empty
   * `<div id="app">`.
   */
  it("a component named only as an object property takes a scope", () => {
    const code = compileSource(
      'import { createRootRoute } from "@barqjs/router";\n' +
        'function Layout() { return <div class="l" />; }\n' +
        "export const Route = createRootRoute({ component: Layout });\n",
      "__root.tsx",
    );
    expect(code).toContain("function Layout(_s$");
    expect(code).not.toContain("const _s$ = null");
  });

  /**
   * And the other half of the same rule: a PROPERTY value, never a bare
   * argument. `rows.map(renderRow)` hands a JSX-returning function to a callee
   * that owns its argument list, which is the miscompilation the sibling gate
   * above detects across the whole corpus.
   */
  it("a callback passed positionally is still not a component", () => {
    const code = compileSource(
      "function renderRow(item) { return <li>{item}</li>; }\n" +
        "export default function List(props: { rows: never }) {\n" +
        "  return <ul>{props.rows.map(renderRow)}</ul>;\n" +
        "}\n",
      "list.tsx",
    );
    expect(code).toContain("function renderRow(item");
    expect(code).not.toContain("function renderRow(_s$");
  });

  /**
   * A declaration the author already wrote on the emitted ABI.
   *
   * `packages/testing`'s wrapper is `(scope, props)` because `render` calls it
   * that way, and it is handed over as `{ wrapper: ThemeWrapper }` — which is
   * the same evidence a route component gives. Prepending a second scope shifts
   * every later parameter along, so `props` lands where the scope was and
   * `props.children` reads the scope's.
   */
  it("a declaration that already takes a scope is not given a second one", () => {
    const code = compileSource(
      'import type { Scope } from "@barqjs/core";\n' +
        "function Wrapper(_s: Scope | null, props: { children: unknown }) {\n" +
        '  return <div data-testid="w">{props.children}</div>;\n' +
        "}\n" +
        "export const mounted = mount({ wrapper: Wrapper });\n",
      "wrapper.tsx",
    );
    expect(code).toContain("function Wrapper(_s");
    expect(code).not.toContain("function Wrapper(_s$, _s");
  });
...
describe("compiler contract", () => {
  ...
  it("a hole's emitted code maps back to the original JSX expression", () => {
    ...
    const hit = onInsert.find((s) => generated[s.genLine].slice(s.genCol).startsWith("clicks()"));
  ...
  it("the inside of a hoisted template maps to the JSX elements that produced it", () => {
    ...
    expect(original[head?.srcLine ?? -1].slice(head?.srcCol ?? 0)).toStartWith(
      '<div class="cell">',
    );
  ...
  it("the first byte inside the literal maps to the template's root element", () => {
    ...
    expect(
      head,
      `no segment on the first byte inside the literal: ${generated[cell]}`,
    ).toBeDefined();
    expect(original[head?.srcLine ?? -1].slice(head?.srcCol ?? 0)).toStartWith(
      '<div class="cell">',
    );
  ...
  it("a deduped template maps to its claimant, and every site keeps its clone call", () => {
    ...
    const claimed = segments.find(
      (s) =>
        s.genLine === cell && generated[s.genLine].slice(s.genCol).startsWith("<span>x</span>"),
    );
