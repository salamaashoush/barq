...
export function chromePath(explicit?: string): string {
  ...
  const candidate = explicit ?? (fromArgv === -1 ? undefined : process.argv[fromArgv + 1]) ??
...
interface Connection {
  send(method: string, params: unknown, sessionId?: string): Promise<{ result?: Record<string, unknown> }>
  close(): void
}
