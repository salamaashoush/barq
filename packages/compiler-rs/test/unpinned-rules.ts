/**
 * The rules the rule set defines and nothing executable observes.
 *
 * the pinning discipline calls the pinning "bidirectionally machine-checkable" and it was only
 ...
 * unpinned set was tracked by prose, which is to say by hand.
 *
 ...
 * which is what the worklist is for. The number is printed by the L1 banner
 ...
 *
 * The M4b gate round struck one more: `O4.5`, whose cell read "structural
 * " — the SIGNATURE was the evidence. `sem-own-given-scope-wins` makes the
 ...
 *
 * The M4 fix round struck seven more, all by writing the fixture already
 ...
 * K1's default REVERSED to identity keying (the wire split Q3), so the rule
 ...
 * `sem-own-slot-arguments` — the one identified as the pin for the
 ...
 * sibling's evidence is the shape the pinning discipline forbids. So every named pin is now
...
export const FICTION_PINS: readonly string[] = Object.freeze([
  ...
  // All seven of E2's entry-point fixtures; M5 wrote the handler one
...
export const UNPINNED_RULES: readonly string[] = Object.freeze([
  ...
  // emit, and the M11 table enumerates why. But clause (f)'s READ SURFACE is a
  ...
  "E3",
  "E4",
  // H — EMPTY. M6 struck H5 off by building the channel the reach table named (the
  ...
  // either side of the fix. the reach table records both.
  "R3",
  "R4",
  "R5",
  "R6",
