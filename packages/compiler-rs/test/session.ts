  ...
  for (const owner of listenerOwners()) {
    ...
    host.removeEventListener = function patchedRemove(
      ...
      for (let i = live.length - 1; i >= 0; i--) {
        ...
        if (record.type !== type || record.listener !== listener || record.capture !== capture) continue
        live.splice(i, 1)
        break
      }
...
function installAsyncProbe(): AsyncProbe {
  ...
  globalThis.setTimeout = ((fn: unknown, ms?: number, ...rest: unknown[]) => {
    if (typeof fn !== "function") return realTimeout(fn as never, ms as never, ...(rest as never[]))
