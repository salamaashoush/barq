...
describe("compile-time addresses", () => {
  ...
  it("-O0 addresses a superset of -Ox, and folding is the whole difference", () => {
    ...
    let extra = 0
    for (const name of CORPUS) {
      const ox = new Set(table(name).positions.map((p) => `${p.kind} ${p.key} ${p.start}-${p.end}`))
    ...
    expect(extra, "no fixture folds a position away, so this row measures nothing").toBeGreaterThan(0)
  })
