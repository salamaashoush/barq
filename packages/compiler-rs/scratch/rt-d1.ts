...
function run(label: string, code: string, file: string) {
  for (const env of ["client", "server"] as const) {
    ...
    const warns = (out.warnings ?? []).map(
      (w: any) => `${w.code ?? "?"}:${w.severity ?? "?"}:${(w.message || "").slice(0, 80)}`,
    );
...
run(
  "A2 export default createServerFn (alone)",
  `
import { createServerFn } from "@barqjs/start";
import { db } from "./db.ts";
...
run(
  "A4 no-jsx route file w/ loader + other export",
  `
import { createServerFn } from "@barqjs/start";
...
run(
  "A6 indirect export of server fn",
  `
import { createServerFn } from "@barqjs/start";
import { db } from "./db.ts";
