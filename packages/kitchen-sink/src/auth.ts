 ...
 * mechanism: a middleware has no build-visible identity of its own, and every
 * attempt to read `.middleware([…])` out of source dies on the shapes people
 * actually write — `[m]`, `[...chain]`, `chain.filter(Boolean)`. Reference
 * identity resolves all of them and needs no compiler work.
 */

import { type SessionConfig, createMiddleware, useSession } from "@barqjs/start";
 ...
 *
 * BUILT WITH `createMiddleware`, not written as a bare closure, and that is
 * required rather than stylistic: the compiler carries a server function's
 * chain into its client stub so a `.client()` half can run, and only a built
 * middleware can have its `.server(…)` body — and the `useSession` import
 * below — deleted on the way. `verifyClientChains` fails the build on the
 * other shape.
 */
export const requireSession = createMiddleware().server(async ({ next }) => {
  ...
  //
  // `sendContext` is the other direction: it rides back on the response, so the
  // page learns who the server decided the user is without a second call.
  return next({
    context: { session: { user: session.data.user ?? "ada" } },
    sendContext: { user: session.data.user ?? "ada" },
  });
});
